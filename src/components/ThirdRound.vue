      ...
      <div class="space-y-4 md:space-y-0 md:grid-cols-9 md:grid-rows-4 md:gap-4 md:grid">
        ...
        <div class="h-full col-span-5 row-span-2 pt-4 text-center bg-white rounded-card">
          <div class="px-8">
            ...
            <div class="pt-6">
              <span class="text-sm italic text-gray-500">Lettres non révélées</span>
              <div class="flex flex-wrap justify-center gap-2 pt-1">
                <span class="px-2 py-1 text-2xl font-medium uppercase rounded text-brown bg-amber-200" v-for="(letter, index) in unrevealedLetters" :key="index">{{ letter }}</span>
              </div>
            </div>
            <div class="flex flex-col pt-6 space-y-4">
              <div class="px-2 mx-auto rounded-md bg-amber-50 w-fit">
                <span class="text-xs font-bold text-gray-500 uppercase">{{ catGram }}</span>
                <span class="text-xs font-medium text-gray-500"> ({{ revealedWord.length }} lettres)</span>
              </div>
              <span class="text-2xl tracking-[0.4em] text-brown uppercase">{{ revealedWord }}</span>
            </div>
            <div class="px-8 pt-8">
              <input
                v-model="userMessage"
        ...
        <div class="col-span-4 row-span-2 text-center rounded-card bg-[url('/public/images/basket.png')] bg-top h-full min-h-36">
          <div
